...
from FashionMNIST.data import get_trainval_datasets, get_dataloader, unnormalize, \
                              NDIMS_TOTAL, IM_DIMS
...
def main(config):
    ...
    std_min = train_config["softflow"]["std_min"]
    std_max = train_config["softflow"]["std_max"]
    std_val = train_config["softflow"]["std_val"]
    ...
    for epoch in range(train_config["n_epochs"]):
        for i, (x, y) in enumerate(train_dl):
            ...
            # We perform augmentation as described in
            # SoftFlow: Probabilistic Framework for Normalizing Flow on Manifolds
            # (https://arxiv.org/pdf/2006.04604.pdf)
            std = ((std_max-std_min) * torch.rand(x.shape[0]) + std_min).to(x.device)
            v = torch.normal(0, std.reshape((-1, 1, 1, 1)).repeat(1, *x.shape[1:]))
            x_aug = x + v

            z, log_j = cinn(x_aug, y, std)
            ...
            if not i % 50:
                with torch.no_grad():
                    ...
                    for (x_val, y_val) in val_dl:
                        ...
                        z, log_j = cinn(x_val, y_val, torch.full_like(y_val, std_val))
